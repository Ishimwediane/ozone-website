      ...
      <div className="container-custom">
          ...
          <Link href="/" className="flex items-center space-x-3 group">
            ...
            <div className="flex flex-col">
              ...
              <span className={`text-xs md:text-sm font-medium transition-colors duration-300 ${
                isScrolled ? "text-[--orange-accent]" : "text-[--orange-400]"
              }`}>
                ENTERTAINMENTS
              </span>
            </div>
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className={`font-medium transition-all duration-300 hover:text-[--orange-accent] animated-underline ${
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden mt-4 pb-4 animate-fadeInDown">
            <div className="flex flex-col space-y-3 bg-white rounded-lg shadow-xl p-4">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="text-gray-700 hover:text-[--orange-accent] font-medium py-2 px-3 rounded-lg hover:bg-gray-50 transition-all duration-300"
                >
                  {link.name}
                </Link>
              ))}
